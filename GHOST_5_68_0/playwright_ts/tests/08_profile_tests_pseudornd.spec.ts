...
test.describe.serial('Random scenarios using faker in profile section ', () =>{
    ...
    test('Text too long on full name field', async({ page }) => {
        ...
        const response = (await fetch('https://my.api.mockaroo.com/week_8.json?key=8f077340'))
        const data = await response.json();
        const apiData: Array<{ text: string }> = data as Array<{ text: string }>
        
        //Given
        await page.goto(`${url}settings/staff/${ghostSlug}`);
        const fakeTitle: string = apiData[0].text.slice(0, 501);
        //When 
        ...
        await page.screenshot({path: './screenshots/profile/pseudo/FullName.png'})   
    })  
    test('Valid email address', async({ page }) => {
        ...
        //Given
        await page.goto(`${url}settings/staff/${ghostSlug}`);
        const fakeTitle: string = apiData[1].text.slice(0, 501);        //When 
        await page.getByPlaceholder('jamie@example.com').fill(fakeTitle);
        //And
        await page.locator('div.pa5').first().click();
        await page.screenshot({path: './screenshots/profile/pseudo/validEmail.png'})   
        //Then
        try {
    ...
    test('location is too long', async({ page }) => {
        ...
        //Given
        await page.goto(`${url}settings/staff/${ghostSlug}`);
        const fakeTitle: string = apiData[2].text.slice(0, 501);        //When 
        //When 
        ...
        await page.screenshot({path: './screenshots/profile/pseudo/longLocation.png'})   
    })
    test('website is not a valid url', async({ page }) => {
        ...
        //Given
        await page.goto(`${url}settings/staff/${ghostSlug}`);
        const fakeTitle: string = apiData[3].text.slice(0, 501);        //When 
        //When
        ...
        await page.screenshot({path: './screenshots/profile/pseudo/validWebsite.png'})   
    })
    test('valid twitter user', async({ page }) => {
        ...
        //Given
        await page.goto(`${url}settings/staff/${ghostSlug}`);
        const fakeTitle: string = apiData[4].text.slice(0, 501);        //When 
        //When
        ...
        await page.screenshot({path: './screenshots/profile/pseudo/validTwitter.png'})   
    })
    test('200 chars in bio', async({ page }) => {
        ...
        //Given
        await page.goto(`${url}settings/staff/${ghostSlug}`);
        const fakeTitle: string = apiData[5].text.slice(0, 501);        //When 
        //When
        ...
        await page.screenshot({path: './screenshots/profile/pseudo/200charsBio.png'})   
    })
    test('+200 chars in bio', async({ page }) => {
        const url: string = 'http://localhost:2368/ghost/#/';
        const ghostSlug: string = 'juanc_villada'
        ...
        //Given
        await page.goto(`${url}settings/staff/${ghostSlug}`);
        const fakeTitle: string = apiData[6].text.slice(0, 501);        //When 
        //When
        await page.locator('textarea[id="user-bio"]').fill(fakeTitle);
        await page.locator('div.pa5').first().click();
        //Then
