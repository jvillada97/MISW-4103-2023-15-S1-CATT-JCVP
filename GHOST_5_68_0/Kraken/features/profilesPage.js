...
class ProfilesPage {
    ...
    async checkIfUpdatedPassword() {
        let element = await this.driver.$("p[data-test-error='user-new-pass']").getText();
    ...
    async checkIfUpdatedPasswordOk() {
        let element = await this.driver.$("span[data-test-task-button-state='success']").getText();
    ...
    async checkIfUpdatedProfile() {
        ...
        if (element == "Password must be at least 10 characters long." && element2 == "Please supply a valid email address") {
            return true;
        }
    }
